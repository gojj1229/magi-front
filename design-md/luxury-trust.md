...
**Trinity 가드 (Balthasar — 안전 / Casper — 사용성)**: ① **verification 카테고리 의미 분기** — 정품·본인·SMS·이메일·KYC 등 *모든* verification 시그널은 *오직* `accent` 컬러 + serif 라벨(또는 별도 verification 전용 토큰)로만 표현. `status-success` `status-approved` 같은 *거래 상태* palette를 verification에 차용 금지. 거래 상태(견적 확정·반려·진행)는 status palette, 신원·정품·인증 검증은 verification palette — 두 palette는 의미 축이 다르므로 색이 우연히 비슷해도 토큰을 섞지 않는다. ② KR/JP 양국 가독성을 위해 본문은 무조건 sans (한자·한글 혼용 시 serif 본문은 가독성 치명적 손실). ③ 견적 숫자는 항상 통화 단위와 *분리* (`￥1,200,000` 식으로 단위가 작은 텍스트로).
